...
# Embed the font as a data URI so the Remotion render has it synchronously
# (no fetch race in headless). Generates remotion/interFontData.ts.
if [ -f remotion/fonts/Inter.ttf ]; then
  say "embedding Inter as data URI (remotion/interFontData.ts)"
  python3 - <<'PY'
import base64
b64 = base64.b64encode(open('remotion/fonts/Inter.ttf','rb').read()).decode()
open('remotion/interFontData.ts','w').write(
  '// Auto-generated by setup.sh: Inter variable font as a data URI.\n'
  'export const INTER_DATA_URI =\n'
  f"  'data:font/ttf;base64,{b64}';\n")
PY
fi
